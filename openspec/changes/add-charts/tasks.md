...
- [ ] 1.1 Read FR-CHART-01 through FR-CHART-05 in spec.md
- [ ] 1.2 Confirm recharts is installed: `ls node_modules/recharts` (installed in scaffold)
- [ ] 1.3 Confirm 178 tests pass on main before starting

## 2. RED tests (write first, confirm they FAIL)

- [ ] 2.1 Write `lib/charts/aggregate.test.ts` (`@trace FR-CHART-01, FR-CHART-02, FR-CHART-03`):
...
- [ ] 2.2 Run `npm run test:run` — confirm new tests FAIL with "Cannot find module"

## 3. Implementation

- [ ] 3.1 Write `lib/charts/types.ts` — `DonutSlice`, `MonthBar` interfaces (from design KD-9)

- [ ] 3.2 Write `lib/charts/aggregate.ts` — pure functions:
...
- [ ] 3.4 Write `app/api/charts/data/route.ts` (GET):
...
- [ ] 3.5 Write `components/charts/ChartSkeleton.tsx` — animated Tailwind placeholder

- [ ] 3.6 Write `components/charts/SpendingDonutChart.tsx` (`'use client'`):
...
- [ ] 3.7 Write `components/charts/MonthlyBarChart.tsx` (`'use client'`):
...
- [ ] 3.8 Write dashboard wrapper `components/charts/ChartsDashboard.tsx` (`'use client'`):
...
- [ ] 3.9 Update `app/page.tsx` — render `<ChartsDashboard />` below budget and transaction sections

## 4. Gate

- [ ] 4.1 `npm run test:run` — all tests pass (target ≥ 190 total)
- [ ] 4.2 `npm run lint` — 0 errors
- [ ] 4.3 `npm run build` — must succeed; check for Recharts SSR warnings
