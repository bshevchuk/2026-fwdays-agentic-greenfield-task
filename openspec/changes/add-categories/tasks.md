...
- [x] 1.1 Create `lib/db/migrations/002_categories.sql`:
...
- [x] 2.1 Create `lib/categories/validation.test.ts` (`@trace FR-CAT-01, FR-CAT-02`):
  - `validateName('')` → false
  - `validateName('  ')` → false
...
- [x] 2.2 Create `lib/categories/service.test.ts` (`@trace FR-CAT-01, FR-CAT-02, FR-CAT-03, FR-CAT-04`):
...
- [x] 2.3 Create `lib/categories/seed.test.ts` (`@trace FR-CAT-04`):
...
- [x] 3.1 Create `lib/categories/types.ts` — `CategoryFullRow`, `CreateCategoryInput`, `UpdateCategoryInput`
- [x] 3.2 Create `lib/categories/validation.ts` — `ALLOWED_ICONS` const (30 icons), `validateName`, `validateColor`, `validateIcon`
- [x] 3.3 Create `lib/categories/queries.ts` — SQL strings for list/get/create/update/delete
- [x] 3.4 Create `lib/categories/service.ts` — service functions returning `{ ok, data | code, error }` discriminated union
- [x] 3.5 Extend `lib/db/repository.ts`:
...
- [x] 3.6 Implement all new methods in `lib/db/adapters/sqlite.ts`
- [x] 3.7 Add category i18n keys to `lib/i18n/en.ts`:
...
- [x] 3.8 Create `app/api/categories/route.ts` — GET (list) + POST (create)
- [x] 3.9 Create `app/api/categories/[id]/route.ts` — GET (single), PUT (partial update), DELETE
- [x] 3.10 Create `lib/categories/` UI components:
...
- [x] 3.11 Create `app/settings/page.tsx` — Server Component; fetches categories list; mounts `CategoryList`
- [x] 3.12 Add Settings link to `components/TopBar.tsx` (link to `/settings`)

## 4. Quality checks

- [x] 4.1 `npm run test:run` — all tests green (including existing shell tests)
- [x] 4.2 `npm run lint` — clean
- [x] 4.3 `npm run build` — clean
- [x] 4.4 `node scripts/check-traceability.mjs` — 0 failures
- [x] 4.5 Smoke: GET /api/categories returns 7 default categories on fresh DB
- [x] 4.6 Smoke: POST → PUT → DELETE round-trip with real SQLite (using `npm run test:run`)
- [x] 4.7 Smoke: DELETE a category with transactions → 422 with correct error string
