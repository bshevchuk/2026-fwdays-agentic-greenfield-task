...
- [ ] 1.1 Read spec FR-BUDGET-01 through FR-BUDGET-05
- [ ] 1.2 Confirm `budget_limit REAL` column exists in 002_categories.sql — no new migration needed
- [ ] 1.3 Confirm 169 tests pass on main before starting

## 2. RED tests (write first, confirm they FAIL)

- [ ] 2.1 Write `lib/budget/status.test.ts` (`@trace FR-BUDGET-05`):
...
- [ ] 2.2 Run `npm run test:run` — confirm new tests FAIL with "Cannot find module" or equivalent

## 3. Implementation

- [ ] 3.1 Write `lib/budget/status.ts`:
...
- [ ] 3.3 Add `PUT /api/categories/[id]/limit` Route Handler at `app/api/categories/[id]/limit/route.ts`:
...
- [ ] 3.6 Update `app/page.tsx` to render `<BudgetDashboard />` alongside `<TransactionList />`

## 4. Gate

- [ ] 4.1 `npm run test:run` — all tests pass (target ≥ 178)
- [ ] 4.2 `npm run lint` — 0 errors
- [ ] 4.3 `npm run build` — success
